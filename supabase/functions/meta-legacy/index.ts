...
const ADD_TO_CART_ACTIONS = ["add_to_cart", "omni_add_to_cart", "offsite_conversion.fb_pixel_add_to_cart"];
const CHECKOUT_ACTIONS = ["initiate_checkout", "omni_initiated_checkout", "offsite_conversion.fb_pixel_initiate_checkout"];
const OUTBOUND_CLICK_ACTIONS = ["outbound_click"];

type LegacyMetricSnapshot = Record<string, number>;
...
// Meta can return the same conversion under more than one legacy action type
// (for example `purchase` and an offsite/omni alias). Choosing the first
// available canonical type prevents the UI from adding aliases and showing
// a value that is larger than the value Meta reports.
function exactActionValue(rows: unknown, names: string[]): number {
  ...
  rows.forEach((row) => {
    ...
    if (actionType && !values.has(actionType)) {
      values.set(actionType, Number(item.value ?? 0));
    }
  });
  for (const name of names) {
    if (values.has(name)) return values.get(name) ?? 0;
  }
  return 0;
}

function metaMetricSnapshot(row: JsonObject): LegacyMetricSnapshot {
  return {
    ...
    outbound_clicks: exactActionValue(row.outbound_clicks, OUTBOUND_CLICK_ACTIONS),
    unique_clicks: Number(row.unique_clicks ?? 0),
    ctr: Number(row.ctr ?? 0),
    cpc: Number(row.cpc ?? 0),
    cpm: Number(row.cpm ?? 0),
    frequency: Number(row.frequency ?? 0),
    leads: exactActionValue(row.actions, LEAD_ACTIONS),
    purchases: exactActionValue(row.actions, PURCHASE_ACTIONS),
    add_to_cart: exactActionValue(row.actions, ADD_TO_CART_ACTIONS),
    initiate_checkout: exactActionValue(row.actions, CHECKOUT_ACTIONS),
    cost_per_lead: exactActionValue(row.cost_per_action_type, LEAD_ACTIONS),
    cost_per_purchase: exactActionValue(row.cost_per_action_type, PURCHASE_ACTIONS),
  };
}

function exactResultValue(rows: unknown): number {
  return exactActionValue(rows, RESULT_ACTIONS);
}

function insightsFields() {
  return "campaign_id,campaign_name,spend,reach,impressions,clicks,inline_link_clicks,unique_clicks,ctr,cpc,cpm,frequency,actions,cost_per_action_type,outbound_clicks";
}
...
function accountOption(account: JsonObject): AvailableAdAccount | null {
  try {
    const adAccountId = normalizeAdAccountId(account.id ?? account.account_id);
    return {
      ad_account_id: adAccountId,
      account_name: String(account.name ?? adAccountId).slice(0, 240),
...
async function availableAdAccounts(token: string): Promise<AvailableAdAccount[]> {
  const accounts = await fetchAllPages<JsonObject>("me/adaccounts", {
    token,
    query: {
  ...
  const unique = new Map<string, AvailableAdAccount>();
  accounts.forEach((account) => {
    const option = accountOption(account);
    if (option) unique.set(option.ad_account_id, option);
  });
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "list_ad_accounts") {
      const accounts = await availableAdAccounts(token);
      const current = accountOption({
        id: connection.ad_account_id,
        name: connection.account_name ?? connection.ad_account_id,
        currency: connection.currency,
        timezone_name: connection.timezone_name,
        account_status: connection.account_status,
      });
      if (current && !accounts.some((item) => item.ad_account_id === current.ad_account_id)) {
        accounts.unshift(current);
      }
      return jsonResponse(req, {
        accounts,
        active_ad_account_id: connection.ad_account_id,
      });
    }

    if (action === "switch_ad_account") {
      const requestedId = normalizeAdAccountId(body.ad_account_id);
      const accounts = await availableAdAccounts(token);
      const selected = accounts.find((account) => account.ad_account_id === requestedId);
      if (!selected) {
        throw new MetaError("This token cannot access the selected Meta ad account", 403, "permission");
      }
      if (selected.ad_account_id === connection.ad_account_id) {
        return jsonResponse(req, {
          success: true,
          switched: false,
          connection: publicConnection(connection),
    ...
    if (action === "update_campaign_status") {
      ...
      if (!/^\d{5,40}$/.test(campaignId) || typeof body.enabled !== "boolean") {
        throw new MetaError("Campaign status request is invalid", 400, "validation");
      }
      ...
      if (
        ...
        normalizeAdAccountId(campaign.account_id) !== connection.ad_account_id
      ) {
        throw new MetaError("Campaign is not available in this workspace ad account", 403, "permission");
      }

      const nextStatus = body.enabled ? "ACTIVE" : "PAUSED";
      await metaRequest<JsonObject>(campaignId, {
        token,
        method: "POST",
      ...
      const { error } = await client
        .from("meta_legacy_campaigns")
        .update({ status: nextStatus, synced_at: new Date().toISOString() })
        .eq("workspace_id", workspaceId)
        .eq("connection_id", connection.id)
    ...
    if (action === "overview") {
      ...
      const insights = await fetchAllPages<JsonObject>(
        `${connection.ad_account_id}/insights`,
        {
          ...
          query: {
            level: "account",
            fields: insightsFields().replace("campaign_id,campaign_name,", ""),
            limit: 25,
            ...(since && until
      ...
      return jsonResponse(req, {
        connected: true,
        currency: connection.currency,
        metrics: metaMetricSnapshot(insights.at(0) ?? {}),
      });
    }
    ...
    if (action === "report") {
      ...
      insights.forEach((row) => {
        ...
        spend += amount;
        results += exactResultValue(row.actions);
    ...
    const [campaigns, insights, accountInsights] = await Promise.all([
    ...
    const rows = campaigns.map((campaign) => {
      ...
      const results = exactResultValue(insight.actions);
      const providerCost = exactActionValue(
        insight.cost_per_action_type,
        RESULT_ACTIONS,
      );
      const budgetMinor = campaign.daily_budget ?? campaign.lifetime_budget;
      const spend = Number(insight.spend ?? 0);
      const metrics = metaMetricSnapshot(insight);
      return {
        workspace_id: workspaceId,
        ...
        // Retained for historic dashboard compatibility. The new UI renders
        // the exact Meta-native metrics saved in meta_metrics instead.
        results,
    ...
    return jsonResponse(req, {
      ...
      currency: connection.currency,
      last_sync_at: now,
      overview: metaMetricSnapshot(accountInsights.at(0) ?? {}),
    });
  } catch (error) {
    if (syncWorkspaceId) {
      try {
