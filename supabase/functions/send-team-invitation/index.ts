  ...
  try {
    ...
    if (action === "delete") {
      const { data, error: lookupError } = await adminClient
        ...
        .maybeSingle();
      if (lookupError || !data) throw new Error("Invitation was not found in this workspace");
      ...
      await adminClient.from("team_audit_log").insert({
        ...
        changes: { previous_status: data.status },
      });
      return json(request, { success: true, status: "deleted" });
    }
    ...
    if (action === "delete_all") {
      ...
      await adminClient.from("team_audit_log").insert({
        ...
        actor_id: user.id,
        actor_email: inviter.email,
        action: "invitations_deleted",
        target_type: "invitation",
        changes: { deleted_count: deleted?.length ?? 0 },
      });
      return json(request, { success: true, status: "deleted", deleted_count: deleted?.length ?? 0 });
    }
    ...
    let refererOrigin = "";
    try { refererOrigin = new URL(request.headers.get("Referer") || "").origin; } catch { /* invalid or missing referrer */ }
    const requestOrigin = request.headers.get("Origin") || refererOrigin || Deno.env.get("APP_URL") || "https://www.ecomos.ma";
