-- ─────────────────────────────────────────────────────────────────────────────
-- Migration: Workspace Reset (final, FK-ordered)
-- Explicit dependency order. No FOREACH. No helper functions.
-- ─────────────────────────────────────────────────────────────────────────────

DROP FUNCTION IF EXISTS _ws_clear_col(uuid, text);
DROP FUNCTION IF EXISTS _ws_clear_bool(uuid, text);
DROP FUNCTION IF EXISTS _ws_delete(uuid, text);
DROP FUNCTION IF EXISTS reset_workspace_data(uuid);
...
BEGIN
    ...
    -- Each column is checked individually so missing / dropped columns are skipped.
    FOREACH v_col IN ARRAY ARRAY[
        ...
        IF EXISTS (
            ...
            EXECUTE format('UPDATE public.workspaces SET %I = NULL WHERE id = %L', v_col, p_workspace_id);
        END IF;
    END LOOP;

    FOREACH v_col IN ARRAY ARRAY['coliaty_enabled','google_sheet_autosync','shopify_enabled'] LOOP
        IF EXISTS (
            SELECT 1 FROM information_schema.columns
            WHERE table_schema = 'public' AND table_name = 'workspaces' AND column_name = v_col
        ) THEN
            EXECUTE format('UPDATE public.workspaces SET %I = false WHERE id = %L', v_col, p_workspace_id);
        END IF;
    END LOOP;
    ...
    END IF;
    IF to_regclass('public.google_sheets_credentials') IS NOT NULL THEN
        DELETE FROM public.google_sheets_credentials WHERE workspace_id = p_workspace_id;
    END IF;
    ...
    END IF;
    IF to_regclass('public.whatsapp_automation_rules') IS NOT NULL THEN
        DELETE FROM public.whatsapp_automation_rules WHERE workspace_id = p_workspace_id;
    END IF;
    ...
    END IF;
    IF to_regclass('public.whatsapp_reply_actions') IS NOT NULL THEN
        DELETE FROM public.whatsapp_reply_actions WHERE workspace_id = p_workspace_id;
    END IF;
