...
alter table public.meta_campaigns add column if not exists created_at timestamptz not null default now();
update public.meta_campaigns set name = campaign_name where name is null;
alter table public.meta_campaigns drop constraint if exists meta_campaigns_meta_campaign_id_key;
alter table public.meta_campaigns drop constraint if exists meta_campaigns_meta_campaign_id_workspace_id_key;
create unique index if not exists meta_campaigns_provider_scope_idx
  on public.meta_campaigns(workspace_id, ad_account_id, meta_campaign_id);
...
create index if not exists orders_meta_campaign_idx on public.orders(workspace_id, meta_campaign_id) where meta_campaign_id is not null;
...
-- Erase the old plaintext credential values. Columns remain temporarily for dependent
-- legacy views/functions but no new code writes or reads them.
do $$
begin
  if exists (select 1 from information_schema.columns where table_schema='public' and table_name='workspaces' and column_name='meta_access_token') then
    execute 'update public.workspaces set meta_access_token=null';
    execute 'revoke select(meta_access_token) on public.workspaces from anon, authenticated';
  end if;
  if exists (select 1 from information_schema.columns where table_schema='public' and table_name='workspaces' and column_name='meta_system_user_token') then
    execute 'update public.workspaces set meta_system_user_token=null';
    execute 'revoke select(meta_system_user_token) on public.workspaces from anon, authenticated';
  end if;
  if exists (select 1 from information_schema.columns where table_schema='public' and table_name='workspaces' and column_name='meta_ad_account_id') then
    execute 'update public.workspaces set meta_ad_account_id=null';
  end if;
end $$;

-- Retire the legacy admin RPC shape that returned the former plaintext Meta token.
drop function if exists public.admin_get_all_workspaces();
create function public.admin_get_all_workspaces()
returns table (id uuid, name text, created_at timestamptz, is_active boolean, status text, created_by uuid)
...
$$;
revoke all on function public.admin_get_all_workspaces() from public, anon;
grant execute on function public.admin_get_all_workspaces() to authenticated, service_role;
...
values ('meta-creatives','meta-creatives',false,524288000,array['image/jpeg','image/png','image/webp','image/gif','video/mp4','video/quicktime','video/webm'])
on conflict (id) do update set public=false, file_size_limit=excluded.file_size_limit, allowed_mime_types=excluded.allowed_mime_types;

drop policy if exists meta_creatives_storage_select on storage.objects;
drop policy if exists meta_creatives_storage_insert on storage.objects;
drop policy if exists meta_creatives_storage_update on storage.objects;
drop policy if exists meta_creatives_storage_delete on storage.objects;
create policy meta_creatives_storage_select on storage.objects for select to authenticated
  using (bucket_id='meta-creatives' and public.meta_can_access_workspace(((storage.foldername(name))[1])::uuid));
create policy meta_creatives_storage_insert on storage.objects for insert to authenticated
  with check (bucket_id='meta-creatives' and public.meta_can_manage_workspace(((storage.foldername(name))[1])::uuid));
create policy meta_creatives_storage_update on storage.objects for update to authenticated
  using (bucket_id='meta-creatives' and public.meta_can_manage_workspace(((storage.foldername(name))[1])::uuid))
  with check (bucket_id='meta-creatives' and public.meta_can_manage_workspace(((storage.foldername(name))[1])::uuid));
create policy meta_creatives_storage_delete on storage.objects for delete to authenticated
  using (bucket_id='meta-creatives' and public.meta_can_manage_workspace(((storage.foldername(name))[1])::uuid));
