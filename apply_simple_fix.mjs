...
async function applySimpleFix() {
  console.log("Applying simple fix to accept_workspace_invitation function...");
  
  // First, let's try to get the current function definition
  const { data: functions, error: funcError } = await supabase
    .rpc('get_function_definition', { function_name: 'accept_workspace_invitation' });

  if (funcError) {
    console.log("Cannot get function definition, will try direct update");
  }

  // The fix is simple: just change the COALESCE line
  const fixSQL = `
  ...
  console.log("Please apply this fix manually in the Supabase SQL Editor:");
  ...
  console.log("\nKey changes:");
  console.log("1. Line 108: allowed_sections = coalesce(invitation_row.allowed_sections, ARRAY[]::text[])");
  console.log("2. Lines 105-116: Temporarily disable/enable the self-profile changes trigger");
  console.log("3. Lines 77-81: Temporarily skip team member limit check due to RLS issues");
  console.log("\nBoth columns are text[], so we use ARRAY[]::text[] instead of '[]'::jsonb.");
  console.log("The trigger fix prevents 'You cannot change your own activation or role state' error.");
  console.log("The RLS fix prevents 'SUBSCRIPTION_READ_NOT_AUTHORIZED' error.");
}
