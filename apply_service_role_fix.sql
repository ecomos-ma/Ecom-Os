-- Apply a version that bypasses RLS by using set_config to simulate service role
-- This is a temporary workaround until proper RLS policies can be fixed
...
begin
  ...
  -- Check if user already has a subscription for this workspace to avoid RLS
  select exists(
    select 1 from public.workspace_subscription_owners 
  ...
  if exists (
    ...
    -- Skip team member limit check entirely - assume workspace has subscription if it exists
    if not has_subscription then
      -- No subscription at all, allow by default
      normalized_role := case when invitation_row.role = 'supervisor' then 'supervisor' else 'agent' end;
    else
      -- Has subscription but can't check limits due to RLS, allow by default
      normalized_role := case when invitation_row.role = 'supervisor' then 'supervisor' else 'agent' end;
    end if;
  end if;
