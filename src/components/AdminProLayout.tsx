...
import {
  Activity, Bell, Boxes, BrainCircuit, ChevronDown, ChevronLeft, DollarSign,
...
type NavItem = { to: string; label: string; icon: typeof LayoutDashboard; exact?: boolean; permission?: PlatformPermission };
...
const navGroups: NavGroup[] = [
  ...
  {
    label: "Business", icon: Gauge, items: [
      { to: "/admin/sellers", label: "Sellers", icon: Gauge, permission: "workspaces.read" },
      ...
      { to: "/admin/orders", label: "All orders", icon: ShoppingCart, permission: "orders.read_all" },
      { to: "/admin/products", label: "Products", icon: PackageSearch, permission: "products.read_all" },
      { to: "/admin/campaigns", label: "Campaigns", icon: Activity, permission: "campaigns.read_all" },
    ],
  },
  {
    label: "Customer operations", icon: CircleHelp, items: [
      { to: "/admin/support", label: "Support", icon: CircleHelp, permission: "support.read" },
      { to: "/admin/announcements", label: "Announcements", icon: Bell, permission: "announcements.manage" },
    ],
  },
  {
    label: "Billing", icon: DollarSign, items: [
      { to: "/admin/subscriptions", label: "Subscriptions", icon: ShieldCheck, permission: "billing.read" },
      { to: "/admin/payments", label: "Payments", icon: DollarSign, permission: "billing.read" },
      { to: "/admin/plans", label: "Plans", icon: PanelsTopLeft, permission: "billing.read" },
    ],
  },
  {
    label: "Platform operations", icon: Activity, items: [
      { to: "/admin/operations?tab=integrations", label: "Integrations", icon: Wrench, permission: "health.read" },
      { to: "/admin/ai-tools", label: "AI & providers", icon: Sparkles, permission: "ai.read" },
      { to: "/admin/operations?tab=health", label: "Health", icon: ShieldCheck, permission: "health.read" },
      { to: "/admin/operations?tab=problems", label: "Logs & errors", icon: FileWarning, permission: "logs.read" },
    ],
  },
  {
    label: "Governance", icon: Settings2, items: [
      { to: "/admin/operations?tab=activity", label: "Audit history", icon: Activity, permission: "security.read" },
      { to: "/admin/platform?tab=security", label: "Security", icon: ShieldCheck, permission: "security.read" },
      { to: "/admin/platform", label: "Settings", icon: Settings2, permission: "settings.read" },
    ],
  },
];
    ...
    <div className="flex h-dvh min-h-0 overflow-hidden bg-base text-ink">
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-72 flex-col border-r border-base-border bg-base-surface transition-[transform,width] duration-200 lg:static lg:translate-x-0 ${sidebarWidth} ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex h-16 shrink-0 items-center gap-3 border-b border-base-border px-4">
          ...
          {!collapsed && <div className="min-w-0"><p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-ink-faint">EcomOS</p><h1 className="truncate text-sm font-bold tracking-tight">Control Center</h1></div>}
        ...
        <div className="border-t border-base-border p-3">
          <div className={`flex items-center gap-3 rounded-xl bg-base-raised p-2.5 ${collapsed ? "justify-center" : ""}`}>
            ...
            {!collapsed && <div className="min-w-0 flex-1"><p className="truncate text-xs font-semibold">{profile?.full_name || "Platform Admin"}</p><p className="truncate text-[11px] capitalize text-ink-faint">{authorization.role?.replace(/_/g, " ") || "Platform Admin"}</p></div>}
