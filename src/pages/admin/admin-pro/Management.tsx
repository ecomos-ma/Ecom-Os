...
export function OrdersPage() {
  ...
  return <div className="mx-auto max-w-[1580px] p-4 md:p-6 lg:p-8"><PageHeading eyebrow="Live operations" title="Order Spy" description="Watch every order across every seller in one secure, live feed. Search, filter and inspect the complete customer and fulfillment context." action={<div className="flex gap-2"><button disabled={exporting || !orders.length} onClick={() => void exportAll()} className="inline-flex items-center gap-2 rounded-xl bg-brand-accent px-3 py-2 text-sm font-bold text-white shadow-sm disabled:opacity-50"><Download size={16} /> {exporting ? "Exporting…" : "Export visible"}</button><RefreshButton onClick={() => void load()} loading={loading} /></div>} />
    ...
    {error ? <EmptyState title="Could not load global orders" copy={error} /> : <section className="overflow-hidden rounded-2xl border border-base-border bg-base-surface shadow-sm"><div className="flex items-center justify-between border-b border-base-border px-4 py-3"><p className="text-sm font-black">Platform order feed</p><p className="text-xs text-ink-faint">Page {page + 1} · newest activity first</p></div><div className="overflow-x-auto"><table className="w-full min-w-[1180px] text-left text-sm"><thead className="bg-base-raised/75 text-[10px] font-black uppercase tracking-wider text-ink-faint"><tr><th className="px-4 py-3">Order</th><th className="px-4 py-3">Seller workspace</th><th className="px-4 py-3">Customer</th><th className="px-4 py-3">Phone / city</th><th className="px-4 py-3">Total</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Shipping status</th><th className="px-4 py-3">Created</th><th className="px-4 py-3"></th></tr></thead><tbody>{loading ? <LoadingRow columns={9} /> : orders.length ? orders.map((order) => { const details = order as FounderOrderV2 & Record<string, unknown>; const shippingStatus = String(details.shipping_status ?? details.delivery_status ?? ""); const tracking = String(details.tracking_number ?? details.tracking_code ?? ""); return <tr key={order.id} className="group border-t border-base-border align-middle hover:bg-brand-accent/[0.035]"><td className="px-4 py-3 font-mono text-xs font-black text-brand-accent">#{order.order_number}</td><td className="px-4 py-3"><span className="block max-w-44 truncate font-semibold">{order.workspace_name || "Deleted workspace"}</span><span className="mt-0.5 block text-[10px] text-ink-faint">{order.workspace_id}</span></td><td className="px-4 py-3"><span className="block max-w-40 truncate font-semibold">{order.customer_name || "—"}</span><span className="mt-0.5 block text-[10px] text-ink-faint">{tracking ? `Tracking · ${tracking}` : "No tracking"}</span></td><td className="px-4 py-3 text-xs text-ink-muted"><span className="block whitespace-nowrap">{order.phone || "—"}</span><span className="mt-0.5 block max-w-36 truncate text-[10px] text-ink-faint">{order.city || "No city"}</span></td><td className="px-4 py-3 font-black whitespace-nowrap">{currency.format(Number(order.total || 0))}</td><td className="px-4 py-3"><OrderStatusBadge status={order.status} size="sm" /></td><td className="px-4 py-3"><ShippingStatusBadge status={shippingStatus} size="sm" />{details.whatsapp_status ? <span className="mt-1 block text-[9px] font-bold text-emerald-600">WhatsApp · {String(details.whatsapp_status)}</span> : null}</td><td className="px-4 py-3 text-xs text-ink-muted whitespace-nowrap">{dateTime.format(new Date(order.created_at))}</td><td className="px-4 py-3 text-right"><button onClick={() => selectOrder(order.id)} className="rounded-xl border border-base-border p-2 text-ink-muted transition group-hover:border-brand-accent/30 group-hover:text-brand-accent" aria-label={`Open order ${order.order_number}`}><FileText size={15} /></button></td></tr>; }) : <EmptyRow columns={9} title="No orders match these filters" copy="Change a filter or clear the search." />}</tbody></table></div><Pagination page={page} total={total} limit={limit} loading={loading} onChange={setPage} /></section>}
