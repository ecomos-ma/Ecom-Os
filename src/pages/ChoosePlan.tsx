  ...
  return <Screen>
    <main className="w-full max-w-6xl px-4 py-10 md:px-8">
      ...
      <section className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {(Object.keys(PRICING_PLANS) as PlanTier[]).map((tier) => { const plan = PRICING_PLANS[tier]; return <button key={tier} type="button" onClick={() => setSelected(tier)} className={`text-left rounded-2xl border p-5 transition ${selected === tier ? "border-brand-accent ring-2 ring-brand-accent/20" : "border-base-border"} bg-base-surface`}><div className="flex items-center justify-between"><h2 className="text-lg font-bold">{plan.name}</h2>{selected === tier && <Check size={18} className="text-brand-accent" />}</div><p className="mt-2 min-h-12 text-sm text-ink-muted">{plan.description}</p><p className="mt-5 text-3xl font-bold">{billing === "monthly" ? plan.monthlyPrice : plan.yearlyPrice}<span className="text-sm font-medium text-ink-muted"> MAD/{billing === "monthly" ? "month" : "year"}</span></p><p className="mt-4 text-sm text-ink-muted">{plan.limits.teamMembers} team members · {plan.limits.workspaces === "unlimited" ? "Unlimited" : plan.limits.workspaces} workspace{plan.limits.workspaces === 1 ? "" : "s"}</p></button>; })}
      </section>
      {error && <p className="mt-5 rounded-xl bg-danger/10 p-4 text-sm text-danger">{error}</p>}
      <button type="button" disabled={saving} onClick={() => void choose()} className="mt-7 rounded-xl bg-brand-accent px-5 py-3 text-sm font-bold text-white disabled:opacity-60">{saving ? "Preparing payment…" : "Continue to payment"}</button>
    </main>
  </Screen>;
}
