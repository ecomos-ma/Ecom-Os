...
function MetaIntegrationCard({ onConnectionChange }: { onConnectionChange?: (connected: boolean) => void }) {
  ...
  const run = async (operation: () => Promise<unknown>, success: string, connect = false) => {
    setBusy(true);
    if (connect) setConnectError(null);
    try { await operation(); toast.success(success); await load(); }
    catch (error) {
      const message = connect ? "Unable to start Meta connection." : (error instanceof Error ? error.message : "Meta operation failed");
      if (connect) setConnectError(message);
      toast.error(message);
  ...
  return <>
    <div className="group relative flex h-full flex-col overflow-hidden rounded-[24px] border border-base-border bg-base-surface p-6 shadow-sm shadow-black/[0.02] transition-all duration-150 hover:scale-[1.02] hover:shadow-md">
      ...
      <div className="mt-auto border-t border-base-border/60 pt-4">
        <button onClick={() => connected ? setModalOpen(true) : run(() => metaAdsService.connect("https://www.ecomos.ma/settings?tab=integrations"), "Redirecting to Meta…", true)} disabled={busy} className="h-[38px] w-full rounded-xl bg-brand px-3 text-[13px] font-semibold text-white shadow-sm transition-colors hover:bg-brand/90 disabled:opacity-60">
          {busy ? "Connecting…" : connected ? "Manage connection" : "Connect with Meta"}
        </button>
        {connectError && <p role="alert" className="mt-2 text-center text-xs font-medium text-danger">{connectError}</p>}
      </div>
    </div>

    {modalOpen && <div className="app-modal-backdrop fixed inset-0 flex h-dvh min-h-0 items-center justify-center overflow-hidden p-4">
      <div className="absolute inset-0 bg-black/55 backdrop-blur-sm" onClick={() => setModalOpen(false)} />
      <div className="relative z-10 flex max-h-[calc(100dvh-2rem)] min-h-0 w-full max-w-3xl flex-col overflow-hidden rounded-[28px] border border-base-border bg-base-surface shadow-2xl">
        ...
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto overscroll-contain px-7 py-6">
          {!connected ? <div className="rounded-2xl border border-brand/20 bg-brand/5 p-5"><div className="flex gap-3"><ShieldCheck className="mt-0.5 text-brand" size={20} /><div><div className="font-semibold text-ink">Secure Meta OAuth</div><p className="mt-1 text-[13px] text-ink-muted">Sign in to Meta, approve the requested business and ads permissions, then select your defaults.</p></div></div><button onClick={() => run(() => metaAdsService.connect("https://www.ecomos.ma/settings?tab=integrations"), "Redirecting to Meta…", true)} disabled={busy} className="mt-4 inline-flex items-center gap-2 rounded-xl bg-brand px-4 py-2.5 text-[13px] font-semibold text-white disabled:opacity-60"><ExternalLink size={14} /> {busy ? "Connecting…" : "Connect with Meta"}</button>{connectError && <p role="alert" className="mt-2 text-xs font-medium text-danger">{connectError}</p>}</div> : <>
