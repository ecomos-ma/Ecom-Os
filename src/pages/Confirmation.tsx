import { Filter, List, Maximize2, RefreshCw, Search, SlidersHorizontal, UsersRound } from "lucide-react";
...
export default function Confirmation() {
  ...
  const setViewMode = (mode: "focus" | "queue") => {
    ...
    try { window.localStorage.setItem("confirmation-view-mode", mode); } catch { /* optional preference */ }
  };
  ...
  useEffect(() => {
    if (viewMode !== "focus" || selectedOrder || crm.loading || !crm.orders.length) return;
    openOrder(crm.orders[0]);
  }, [viewMode, selectedOrder?.id, crm.loading, crm.orders]);
  ...
  const saveAndNext = () => {
    ...
    const next = currentIndex >= 0
      ? crm.orders[currentIndex + 1] || crm.orders[currentIndex - 1] || null
      : crm.orders[0] || null;
  ...
  return (
    <div className="space-y-5 pb-8">
      ...
      {viewMode === "queue" && <ConfirmationMetrics summary={crm.summary} loading={crm.loading} myView={!crm.canManage} />}

      {viewMode === "queue" && crm.canManage && crm.agentMetrics.length > 0 && (
      ...
      <section className="space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-base-border bg-base-surface p-2.5 shadow-sm">
          <div className="relative min-w-[220px] flex-1 max-md:min-w-0 max-md:basis-full"><Search size={14} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-ink-muted" /><input value={crm.searchInput} onChange={(event) => crm.setSearchInput(event.target.value)} placeholder="Search order, customer, phone, city, SKU…" className="h-9 w-full rounded-lg bg-base-raised pl-9 pr-3 text-[12px] text-ink placeholder:text-ink-faint outline-none ring-1 ring-transparent focus:ring-brand/35" /></div>
        ...
        {filtersOpen && <div className="hidden flex-wrap items-end gap-3 rounded-xl border border-base-border bg-base-raised/45 p-3.5 md:flex"><div><label className="block text-[10.5px] font-semibold uppercase tracking-[0.07em] text-ink-muted">Date</label><select value={crm.datePreset} onChange={(event) => crm.setDatePreset(event.target.value as typeof crm.datePreset)} className="mt-1.5 h-9 rounded-lg border border-base-border bg-base-surface px-2.5 text-[11.5px] text-ink outline-none focus:border-brand/40"><option value="all">All time</option><option value="today">Today</option><option value="yesterday">Yesterday</option><option value="month">This month</option></select></div>{crm.canManage && <div><label className="block text-[10.5px] font-semibold uppercase tracking-[0.07em] text-ink-muted">Assigned agent</label><select value={crm.assigneeId || ""} onChange={(event) => crm.setAssigneeId(event.target.value || null)} className="mt-1.5 h-9 min-w-[180px] rounded-lg border border-base-border bg-base-surface px-2.5 text-[11.5px] text-ink outline-none focus:border-brand/40"><option value="">All agents</option>{crm.agents.map((agent) => <option key={agent.id} value={agent.id}>{agent.fullName}</option>)}</select></div>}<div className="pb-0.5 text-[11px] text-ink-muted">{crm.total.toLocaleString()} matching order{crm.total === 1 ? "" : "s"}</div></div>}
        <MobileBottomSheet isOpen={filtersOpen} onClose={() => setFiltersOpen(false)} title="Confirmation filters" mobileOnly>
          <div className="space-y-4">
            <label className="block text-xs font-bold uppercase tracking-wide text-ink-faint">Date<select value={crm.datePreset} onChange={(event) => crm.setDatePreset(event.target.value as typeof crm.datePreset)} className="mt-2 min-h-12 w-full rounded-xl border border-base-border bg-base-raised px-3 text-base text-ink"><option value="all">All time</option><option value="today">Today</option><option value="yesterday">Yesterday</option><option value="month">This month</option></select></label>
            {crm.canManage && <label className="block text-xs font-bold uppercase tracking-wide text-ink-faint">Assigned agent<select value={crm.assigneeId || ""} onChange={(event) => crm.setAssigneeId(event.target.value || null)} className="mt-2 min-h-12 w-full rounded-xl border border-base-border bg-base-raised px-3 text-base text-ink"><option value="">All agents</option>{crm.agents.map((agent) => <option key={agent.id} value={agent.id}>{agent.fullName}</option>)}</select></label>}
            <button type="button" onClick={() => setFiltersOpen(false)} className="min-h-12 w-full rounded-xl bg-brand text-sm font-bold text-white">Show {crm.total.toLocaleString()} order{crm.total === 1 ? "" : "s"}</button>
          </div>
        </MobileBottomSheet>
      </section>
